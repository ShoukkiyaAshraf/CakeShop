...
if (isset($_SESSION['user_users_id']) && $_SESSION['user_users_username'] !== null) {
    ...
    // $price_ = $_POST['hidden_product_price_'];
    ...
    for ($i=0; $i < count($product_name); $i++) { 
    	$insert_orders_detail = "INSERT INTO cake_shop_orders_detail (orders_id, product_name, quantity,price) values ('$orders_id', '$product_name[$i]', '$quantity[$i]', '$total[$i]')";
    	mysqli_query($conn, $insert_orders_detail);
    }
    unset($_SESSION['cart']);
    header("Location: cart.php?order_success=1&orders_id=".$orders_id);
} else {
	echo "<script>
	alert('Please login!!!');
